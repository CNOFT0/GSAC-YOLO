...
from timm.models.layers import DropPath#新加faster
...
class SCiAM(nn.Module):
    '''
    Combined SCAM and iAFF module
    '''

    def __init__(self, channels, reduction=1):
        super(SCiAM, self).__init__()
        self.scam = SCAM(channels, reduction)
        self.iaff = iAFF(channels, r=2)

    def forward(self, x, residual=None):
        if residual is None:
            residual = x  # 使用输入作为默认残差
        # Apply SCAM on input feature x
        x_scam = self.scam(x)

        # Apply iAFF on SCAM output and residual
        output = self.iaff(x_scam, residual)
        return output
############新加
...
class GSSPPFCSPC(nn.Module):

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(GSSPPFCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.gscv1 = GSConv(c1, c_, 1, 1)
        self.gscv2 = GSConv(c1, c_, 1, 1)
        self.gscv3 = GSConv(c_, c_, 3, 1)
        self.gscv4 = GSConv(c_, c_, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.gscv5 = GSConv(4 * c_, c_, 1, 1)
        self.gscv6 = GSConv(c_, c_, 3, 1)
        self.gscv7 = GSConv(2 * c_, c2, 1, 1)

    def forward(self, x):
        x1 = self.gscv4(self.gscv3(self.gscv1(x)))
        x2 = self.m(x1)
        x3 = self.m(x2)
        # y1 = self.gscv5(torch.cat((x1, x2, x3, self.m(x3)), 1))  # 使用GS卷积
        # y2 = self.gscv6(x)  # 使用GS卷积处理输入特征
        y1 = self.gscv6(self.gscv5(torch.cat((x1, x2, x3, self.m(x3)), 1)))
        y2 = self.gscv2(x)
        return self.gscv7(torch.cat((y1, y2), dim=1))
###################新加
